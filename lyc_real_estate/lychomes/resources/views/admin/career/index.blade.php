            ...
            <div class="body">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                        ...
                        <tbody>
                            @foreach( $list as $key => $career )
                            <tr>
                                <td>{{$career->id}}</td>
                                <td>
                                    <span title="{{$career->name}}">
                                        {{ str_limit($career->name) }}
                                    </span>
                                </td>
                                <td>
                                    <span title="{{$career->intro}}">
                                        {{ str_limit($career->intro) }}
                                    </span>
                                <td>
                                    {{ $career->salary }}
                                </td>
                                <td>
                                    {{$career->updated_at->format('Y-m-d H:i:s')}}
                                </td>
                                ...
                                <td class="text-center">
                                    ...
                                    <button type="button" class="btn btn-danger btn-sm waves-effect"
                                        onclick="deletePost({{$career->id}})">
                                        <i class="material-icons">delete</i>
                                    </button>
                                    <form action="{{route('admin.career.destroy',$career->id)}}" method="POST"
                                        id="del-post-{{$career->id}}" style="display:none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
