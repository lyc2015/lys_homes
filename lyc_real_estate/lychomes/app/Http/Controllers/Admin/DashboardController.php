...
class DashboardController extends Controller
{
    ...
    {
        $rentcount = Property::where("purpose", "rent")->count();
        $salecount = Property::where("purpose", "sale")->count();
        $usercount     = User::count();

        $rents    = Property::where("purpose", "rent")->latest()->with('user')->take(5)->get();
        $sales    = Property::where("purpose", "sale")->latest()->with('user')->take(5)->get();
        $users         = User::with('role')->get();

        return view('admin.dashboard2', compact(
            'rentcount', "salecount", 'usercount',
            'rents', "sales",     'users'
        ));
    }
    ...
    public function video(Request $request)
    {
        if ($request->isMethod(Request::METHOD_POST)) {
            ...
            $videos = array_values(array_diff(scandir($videosDir), array('.', '..')));
