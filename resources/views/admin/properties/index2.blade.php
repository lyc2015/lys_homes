...
<div class="block-header">
    <a href="{{route('admin.properties.create', ['purpose' => $purpose,])}}"
                    ...
                    <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                        <thead>
                            <tr>
                                ...
                                <th>物件名</th>
                                <th width="300">交通</th>
                                <th width="150">{{ $purpose == "rent" ? "賃貸価格" : "販売価格" }}</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span title="{{$property->title}}">
                                        {{ str_limit($property->title) }}
                                    </span>
                                </td>
                                <td>
                                    <span title="{{$property->nearby}}">
                                        {{ str_limit($property->nearby) }}
                                    </span>
                                <td>
                                    {{ $property->price }} 万円
                                </td>
                                ...
                                <td class="text-center">
                                    ...
                                    <form action="{{route('admin.properties.destroy',$property->id)}}" method="POST"
                                        id="del-post-{{$property->id}}" style="display:none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
